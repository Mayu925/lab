...
get_prm_list <- function(param, i, r, c){
  return(list(alpha_i = param$alpha_i[i], alpha_r = param$alpha_r[r], alpha_c = param$alpha_c[c], 
              beta_i = param$beta_i[i], beta_r = param$beta_r[r], beta_c = param$beta_c[c],
              tau_r = param$tau_r[r], category_prm = append(0, param$beta_ck[c,])))
}

logit <- function(param, category_prm, x){
  return(param$alpha_i * param$alpha_r * param$alpha_c 
