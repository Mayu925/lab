...
est_param_new_1 <- get_estimates(fit1, setting)
D_new_1 <- get_result_statistics_common(fit1, data1, setting)
write.csv(t(matrix(D_new_1, nrow=2)), paste(dir, "output/realdata/MCMC_statistics/mayu/", model, "1.csv", sep=""))
write.csv(t(matrix(est_param_new_1$alpha_rt, nrow=34)), paste(dir, "output/realdata/parameters/mayu/", model, "1.csv", sep=""))
