read_data <- function(setting, filename){
  ...
  colnames(data) <- c("ExamineeID", "ItemID", "RaterID", "RubricID", "Score")
  data_irt=list(
    K=setting$K, J=setting$n_person, I=setting$n_item, R=setting$n_rater, C=setting$n_rubric, N=nrow(data), 
...
to_wide_format <- function(data){
  ...
  for (n in 1:length(data$X)){
    X[data$ExamineeID[n], data$ItemID[n], data$RaterID[n],data$RubricID[n]] = data$X[n];
  }
  return(X)
}
...
to_long_format <- function(data, X){
  for (n in 1:length(data$X)){
    data$X[n] <- X[data$ExamineeID[n], data$ItemID[n], data$RaterID[n],data$RubricID[n]];
  }
  return(data)
}
