...
write.csv(t(matrix(est_param1, nrow=34)), paste(dir, "output/realdata/parameters/mayu/previous/", model, "t_3.csv", sep=""))

est_param2 <- get_estimates(fit2, setting2)
D2 <- get_result_statistics_common(fit2, data2, setting2)
write.csv(t(matrix(D2, nrow=2)), paste(dir, "output/realdata/MCMC_mayu/previous/", model, "t_5.csv", sep=""))
