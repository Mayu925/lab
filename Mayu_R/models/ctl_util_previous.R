...
generate_data <- function(param, setting){
  ...
  colnames(U) <- c("ExamineeID", "RaterID","TimeID","Score")
  data <- data.frame(U)
  data=list(K=setting$K, J=setting$n_person,R=setting$n_rater, T=setting$n_time, N=nrow(data), 
             ExamineeID=data$ExamineeID, RaterID=data$RaterID, TimeID=data$TimeID, X=data$Score)
  return(data)
}
...
draw_icc <- function(param, K, def){
  ...
  for(k in 2:K){
    ...
    par(new=T)
    plot(x, prob(param, k, x, t), xlim=c(-def$xlim, def$xlim), ylim=c(0,def$ylim), xlab="", ylab="",
