...
これらのモデルは評価者の基準が採点過程で変化しないという仮定のもと成り立っているが，その仮定は現実には成り立たないことがある．この評価者の採点基準が採点過程で変化する特性は評価者ドリフト(Rater Drift)と呼ばれ，これを考慮したモデルについても提案がなされている．既存モデルでは，評価者の厳しさの初期値と傾きを表すパラメータを導入することで，評価者特性の時間変化を捉えるモデルとなっているが，このモデルでは各時間区分ごとのパラメータが独立しているため，パラメータの推定が難しいと言う問題点に加え，評価者特性の変化を直線的にしかとらえることが出来ないという問題点がある．この問題を解決するために，本研究では，時間区分ごとの評価者の厳しさにマルコフ性を仮定した新しい項目反応モデルを提案する．また，シミュレーション実験と実データ実験を通して提案モデルの有効性を示す．
\tableofcontents
%
%
\mainmatter
...
この問題を解決するために，評価者特性の時間変動を考慮した項目反応モデルが提案されている\cite{raterdrift}．このモデルは，評価者が行なった一連の評価を時間区分で区切り，評価者の初期の厳しさと厳しさの傾きを使用して評価者特性の時間変化を捉えるモデルとなっている．
しかし，既存モデルの問題点として次の点が挙げられる．
\begin{enumerate}
  \item 時間区分ごとのパラメータが独立しているため，パラメータの推定が難しい
...
\begin{enumerate}
  ...
  \item 時間区分ごとの評価者特性の変化をより細かく推定することができる
...
そのうち，6人には採点の際に指示を与え，人為的に評価者バイアスのあるデータを作成するようにした．以下では，その6人をそれぞれ，評価者A，B，C，D，E，Fとする．それぞれに与えた指示は以下の通りである．
...
また，評価者Dと評価者Eの評価者パラメータから作成した項目反応曲線を表\ref{ICC_DE}に示す．これを見ると，どちらも多く使うように指示した得点を多く使用していることがデータから推定できていることがわかる．
...
\centering
 ...
 \end{minipage}
 \caption{指示を与えた評価者の項目反応曲線}\label{ICC_DE}
\end{figure}
...
表\ref{WAIC1}と\ref{WAIC2}から，各時間区分におけるWAICの最小値を比較すると，提案モデルが最適モデルとして選択されたことが確認できる．
...
次に，表\ref{WBIC1}と\ref{WBIC2}から，各時間区分におけるWBICの最小値を比較すると，提案w/o$\alpha_r$と提案モデル2が最も高い性能を示しており，提案モデルより単純なモデルが最適なモデルとして選択されていることがわかる．一方で，提案モデルはどちらにおいても2番目に高い性能を示しており，$\beta_{rt}$にマルコフ性を導入したことの有効性は確認できる．
...
\begin{table*}[tb]
    \begin{center}
    \caption{WAICによるモデル比較の結果(指示なしの評価者)}
    \setlength{\tabcolsep}{5.pt}
    \begin{tabular}{cccccccc}  
      \bhline{1pt}
      既存モデル & 提案モデル & 提案モデル2 & 提案モデル3& 提案w/o $\alpha_r$\\ 
      \bhline{1pt}
      3279.444 & \textbf{3134.7} & 3190.802 & 3154.331 & 3137.137\\
      \bhline{1pt}
    \end{tabular}
    \label{WAIC2}
  \end{center}
...
\begin{table*}[tb]
  \begin{center}
    \caption{WBICによるモデル比較の結果(指示なしの評価者)}
      \setlength{\tabcolsep}{5.pt}
      \begin{tabular}{cccccccc}  
      \bhline{1pt}
       既存モデル & 提案モデル & 提案モデル2 & 提案モデル3& 提案w/o $\alpha_r$\\ 
      \bhline{1pt}
      1927.215 & 1900.211 & \textbf{1863.71} & 1924.21& 1910.699\\
      \bhline{1pt}
    \end{tabular}
    \label{WBIC2}
  \end{center}
